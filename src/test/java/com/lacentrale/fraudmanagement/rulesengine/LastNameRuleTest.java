...
public class LastNameRuleTest {
    ...
    LastNameRule lastNameRule = new LastNameRule("Last Name Rule Test");
    ...
    public void should_return_true_when_first_name_length_is_greater_than_2() {
        ...
        assertTrue(lastNameRule.evaluate().test(advertisement));
    }

    @Test
    public void should_return_false_when_first_name_length_is_lower_than_2() {
        Contacts contacts = new Contacts("Abraham", "Is", "az@yopmail.com", null);
        when(advertisement.getContacts()).thenReturn(java.util.Optional.of(contacts));
        assertFalse(lastNameRule.evaluate().test(advertisement));
    }
}
